...
function toWpRelative(p) {
  ...
  if (p.startsWith("http://") || p.startsWith("https://")) {
    const u = new URL(p);
    // kun WP origin eller samme origin må passere (ellers afvis)
...
async function httpJson(url, options = {}) {
  ...
  // Prøv at parse json (også ved fejl), så vi får WP-fejlmeddelelser med
  let data = null;
  try {
    data = await res.json();
  } catch {
...
// ================================
// ✅ Direkte WP fetch via Vercel rewrite (ingen tg/v1/proxy nødvendig)
// Forudsætter at vercel.json har rewrite: /wp-json/(.*) -> https://telegiganten.dk/wp-json/$1
// ================================
...
// ================================
// API (WP endpoints bruger nu wpFetch)
// ================================
export const api = {
  // -----------------
  // Repairs
  ...
  getOrderById: (id) =>
    wpFetch({ path: `/wp-json/telegiganten/v1/orders/${id}` }),
  ...
  spareparts: {
    ...
    create: (part) =>
      primaryThenFallback(() => gasCreate(part), () => wpCreateV2(part)),
